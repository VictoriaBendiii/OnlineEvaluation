...
if(isset($_POST['content'])){
    ...
    if(empty($keyword)){
        ...
        while ($row = $result->fetch_assoc()){
            echo "<div id='$row[username]' draggable='true' class='panel panel-default $row[username]' style='width: 80%; background-color: #EFEEEC; padding: 5px;'><img src='images/profilepictures/$row[profilepicture]' style='border-radius: 50px; height: 50px; width: 50px; padding: 2px;' alt='profile picture'>";
            echo "<p style='font-size: 21px; display: inline-block;'>&nbsp;$row[name]&emsp;</p>

                <input type='hidden' name='id' value='$row[id]'>
                <input type='hidden' name='course' value='$course'><br></div>";

            echo "<script>if($('#groupCont').children().find('#$row[username]').length == 1){
            $('#stds').children('#$row[username]').remove();
            }</script>";
        }
    }else{
        $sql = "SELECT users.id as id, CONCAT(users.firstname,' ',users.lastname) as name, users.username as username, users.profilepicture as profilepicture FROM users JOIN user_course USING (id) WHERE user_course.courseCode = '$course' AND users.identification='student';";
